...
| 2 무엇을 만들까 (`기획`) | 아이템 기획 | 아이템 순위표(인기 비중·어제 대비·상품 수·중간 가격·최고 순위, 0.5%p 이상 오른 줄 초록) + 추천 아이템 5개 |
| 3 디자인 참고 (`디자인`) | 아이템별 스펙 | 카테고리 버튼 → 아이템 카드(잘 팔리는 사진 6장, 실루엣·기장(팬츠류만) / 원단 / 핏 / 소재 / 컬러 / 디테일) + 전체 핏·실루엣·디테일 순위 |
| 4 소재·컬러 (`소재컬러`) | 원단 소싱·컬러 기획 | 컬러 팔레트(실제 색 칩) + 원단·소재·두께 순위 |
| 5 가격 (`가격`) | 가격 포지셔닝 | 아이템별 중간 가격·주로 팔리는 가격(가운데 50%) 막대 + 가격대 분포 |
| 6 시장 동향 (`동향`) | 경쟁 상품 모니터링 | 카테고리별 TOP 10(사진) + 순위 급상승 + 새로 TOP 50 진입 |
...
| 핏 | 판매자 입력 핏 (스키니/슬림/레귤러/루즈/오버사이즈) | 상품명 키워드 → 상세 설명 글 키워드 |
...
- 세 주소 모두 robots.txt 없음(404). www.musinsa.com은 등록 안 된 봇을 금지하므로 쓰지 않음.
