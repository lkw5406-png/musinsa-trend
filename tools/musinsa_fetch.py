...
남성·여성 전체 랭킹(일간·주간·월간 중 하나)을 1위부터 내려가며, 의류만 골라 성별마다 300개를 모아 CSV로 저장한다.
저장 위치: 일간 data/history/, 주간 data/history_weekly/, 월간 data/history_monthly/ (common.history_path)
무신사가 알려주는 랭킹 갱신 시각은 data/ranking_updates.json에 기록 (갱신 시각·집계 구간 확인용).
...
사용법: python tools/musinsa_fetch.py [--date YYYY-MM-DD] [--period daily|weekly|monthly]
"""
import argparse
import csv
import json
...
from common import KST, PERIODS, UPDATES_PATH, get_json, history_path, today_kst
...
def build_url(gender: str, page: int, last_rank: int, period: str = "daily") -> str:
    params = {
        "storeCode": "musinsa",
        "gf": gender,
...
def updated_at(data: dict) -> str | None:
    """랭킹 응답 안의 갱신 시각(updatedAt, 밀리초) → '2026-09-25 04:45' (한국시간)."""
    found = []

    def walk(o):
        if isinstance(o, dict):
            if isinstance(o.get("updatedAt"), (int, float)):
                found.append(o["updatedAt"])
            for v in o.values():
                walk(v)
            ...
            for v in o:
                walk(v)
    walk(data)
    return datetime.fromtimestamp(max(found) / 1000, KST).strftime("%Y-%m-%d %H:%M") if found else None


def record_update(period: str, date: str, gender: str, when: str | None) -> None:
    log = json.loads(UPDATES_PATH.read_text(encoding="utf-8")) if UPDATES_PATH.exists() else {}
    log.setdefault(period, {}).setdefault(date, {})[gender] = {
        "updated_at": when, "fetched_at": datetime.now(KST).strftime("%Y-%m-%d %H:%M")}
    UPDATES_PATH.write_text(json.dumps(log, ensure_ascii=False, indent=1, sort_keys=True), encoding="utf-8")


def fetch_gender(gender: str, date: str, details: dict, period: str = "daily") -> list[dict]:
    """전체 랭킹을 한 페이지씩 받아, 의류가 CLOTHING_TARGET개 모일 때까지 내려간다."""
    rows: list[dict] = []
    seen_ranks: set[int] = set()
    last_rank = 0
    for page in range(1, MAX_PAGES + 1):
        data = get_json(build_url(gender, page, last_rank, period))
        time.sleep(REQUEST_INTERVAL_SEC)
        if page == 1:
            record_update(period, date, gender, updated_at(data))
        ...
        for p in products:
            ...
            rows.append({"date": date, "period": PERIODS[period][0], "gender_filter": gender,
...
def collect(date: str, period: str = "daily") -> list[dict]:
    details = product_details.load_details()
    rows = []
    for gender in GENDERS:
        rows += fetch_gender(gender, date, details, period)
    return rows
...
def save_csv(rows: list[dict], date: str, period: str = "daily"):
...
def main() -> int:
    ...
    print(f"무신사 랭킹 수집 시작 ({args.date}, 전체 랭킹 {PERIODS[args.period][2]} · 성별 의류 {CLOTHING_TARGET}개)")
    rows = collect(args.date, args.period)
    validate(rows)
    path = save_csv(rows, args.date, args.period)
